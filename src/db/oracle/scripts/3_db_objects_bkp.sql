...
WHERE
  ...
  GROUP BY  DENTAL_SERVICE.ID,
    ...
    DENTAL_STATUS.DESCRIPTION,TO_CHAR(DENTAL_SERVICE.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS'), DENTAL_SERVICE.ARE_YOU_ELIGIBLE_FOR_THE_PHARMACARE_AND_EXTENDED_HEALTH_CARE_BEN;
...
BEGIN
	...
	IF exist_record > 0 THEN
		...
		INTO oldest_request
		FROM DENTAL.DENTAL_SERVICE
		WHERE STATUS <> 4  AND FIRST_NAME COLLATE BINARY_CI LIKE :NEW.FIRST_NAME AND LAST_NAME COLLATE BINARY_CI LIKE :NEW.LAST_NAME AND DATE_OF_BIRTH  = :NEW.DATE_OF_BIRTH AND  ROWNUM <= 1
		ORDER BY CREATED_AT ;
